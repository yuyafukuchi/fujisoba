...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        <!--原価を出す-->
        <?php $sum_cost_array=array() ?>
        <?php $sum_cost=0 ?>
        <?php for($i = 2 ; $i <= $lastDay ; $i ++) : ?> <!--日で回す-->
            <?php if( empty($inventoryPurchaseTransactionsCostArray[$i])): ?>
                <?php $sum_cost_array[$i]=0 ?>
            <?php else :?>
                <?php for($j= 0 ;$j<count($inventoryPurchaseTransactionsCostArray[$i]) ; $j++): ?>
                    <?php $x=array_keys($inventoryPurchaseTransactionsCostArray[$i][$j]) ?> <!--investory_item_id-->
                    <?php for($h= 0 ;$h<count($storeInventoryItemHistoryCostArray[$i]) ; $h++): ?>
                        <?php if(array_keys($storeInventoryItemHistoryCostArray[$i][$h])[0]==
                        array_keys($inventoryPurchaseTransactionsCostArray[$i][$j])[0]): ?> <!--inventory_item_id-->

                            <?php $sum_cost+=($storeInventoryItemHistoryCostArray[$i][$h][$x[0]])*
                            ($inventoryPurchaseTransactionsCostArray[$i-1][$j][$x[0]][2]
                            +$inventoryPurchaseTransactionsCostArray[$i][$j][$x[0]][0]
                            -$inventoryPurchaseTransactionsCostArray[$i][$j][$x[0]][1]
                            -$inventoryPurchaseTransactionsCostArray[$i][$j][$x[0]][2])?>
                        <?php endif; ?>
                    <?php endfor; ?>
                <?php endfor; ?>
                <?php $sum_cost_array[$i]=$sum_cost ?>
                <?php $sum_cost=0 ?>
            <?php endif; ?>
        <?php endfor?>
        ...
        <!--表を埋めて行く-->
        <?php $fill_check =array_fill(0,$lastDay,0)?>
        <?php for($i = 1 ; $i <= $lastDay ; $i ++) : ?>
            <tr>
                <td><?=$i ?></td>
                ...
                <?php for($j = 0 ; $j < $arraySize ; $j ++) :?>
                    <?php if(!empty($salesDailySummary[$j]['transaction_date'])
                    &&$salesDailySummary[$j]['transaction_date']->i18nFormat('dd') == $i ): ?>
                        ...
                        <td><?= h($sum_cost_array[$i]) ?></td>
                        <?php $fill_check[$i-1]=1 ?>

                    <?php endif; ?>
                <?php endfor; ?>

                <!--値が表に埋められない場合は、空白を追加する-->
                      <?php if($fill_check[$i-1]!=1) : ?>
                           <td></td>
                           <td></td>
                           <td></td>
                           <td></td>
